      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
          <h1 className="text-5xl md:text-6xl font-bold mb-6">
            About <span className="text-pink-400" style={{ fontFamily: '"Pacifico", serif' }}>SIPPR</span>
          </h1>
            ...
            {story.map((chapter, index) => (
              <div key={index} className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${index % 2 === 1 ? 'lg:grid-flow-col-dense' : ''}`}>
                <div className={index % 2 === 1 ? 'lg:col-start-2' : ''}>
                  <h3 className="text-3xl font-bold text-gray-800 mb-4">{chapter.title}</h3>
                ...
                <div className={index % 2 === 1 ? 'lg:col-start-1' : ''}>
                  <img 
                    src={chapter.image}
                    alt={chapter.title}
                    className="rounded-2xl shadow-2xl object-cover object-top w-full h-80"
                  />
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {team.map((member, index) => (
              <Card key={index} hover className="p-8 text-center">
                <div className="w-20 h-20 mx-auto mb-6 bg-[#C34479] rounded-full flex items-center justify-center">
                  <i className={`${member.icon} text-3xl text-white`}></i>
                </div>
                <h3 className="text-xl font-bold text-gray-800 mb-2">{member.name}</h3>
                <h4 className="text-[#C34479] font-semibold mb-4">{member.role}</h4>
